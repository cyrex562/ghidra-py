...
public class FunctionRowObjectToFunctionTableRowMapper extends
		ProgramLocationTableRowMapper<VTFunctionRowObject, Function> {

	@Override
	public Function map(VTFunctionRowObject rowObject, Program data, ServiceProvider serviceProvider) {
		FunctionManager functionManager = data.getFunctionManager();
		FunctionAssociationInfo info = rowObject.getInfo();
		return functionManager.getFunction(info.getFunctionID());
	}
