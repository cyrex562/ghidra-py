...
public class InRangeColumnConstraint<T extends Comparable<T>> extends RangeColumnConstraint<T> {
	 ...
	 */
	public InRangeColumnConstraint(T minValue, T maxValue, EditorProvider<T> editorProvider) {
		this("In Range", minValue, maxValue, editorProvider, "number");
	}
	 ...
	 * and max values for the range.
	 *
	 * @param name the constraint to use instead of the default "In Range".
	 * @param minValue the min value of the acceptable range.
	 * @param maxValue the max value of the acceptable range.
	 * @param editorProvider an object that can provide an appropriate range editor for the column type.
	 * @param group the group to use instead of the default value of "number".
	 */
	public InRangeColumnConstraint(String name, T minValue, T maxValue,
	...
	public RangeColumnConstraint<T> copy(T min, T max) {
		return new InRangeColumnConstraint<>(getName(), min, max, editorProvider, getGroup());
	}
