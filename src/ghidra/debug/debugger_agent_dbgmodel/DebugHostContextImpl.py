...
public class DebugHostContextImpl implements DebugHostContextInternal {
	@SuppressWarnings("unused")
	private final OpaqueCleanable cleanable;
	private final IDebugHostContext jnaData;

	public DebugHostContextImpl(IDebugHostContext jnaData) {
		this.cleanable = DbgModel.releaseWhenPhantom(this, jnaData);
		this.jnaData = jnaData;
	}

	public IDebugHostContext getJnaData() {
		return jnaData;
	}
	...
	@Override
	public boolean isEqualTo(DebugHostContext context) {
		Pointer pContext = context.getPointer();
		BOOLByReference pIsEqual = new BOOLByReference();
		COMUtils.checkRC(jnaData.IsEqualTo(pContext, pIsEqual));
