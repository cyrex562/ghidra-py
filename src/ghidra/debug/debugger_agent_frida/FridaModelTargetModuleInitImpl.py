...
public class FridaModelTargetModuleInitImpl extends FridaModelTargetObjectImpl
		implements FridaModelTargetMethod {

	protected final TargetParameterMap paramDescs;

	public FridaModelTargetModuleInitImpl(FridaModelTargetModuleContainer modules) {
		super(modules.getModel(), modules, "init", "ModuleInit");
	...
	protected Map<String, ParameterDescription<?>> computeParameters() {
		...
		ParameterDescription<String> fpath = ParameterDescription.create(String.class, "Name",
			true, "", "Name", "module name");
		map.put("Name", fpath);
		return map;
	}
	...
	public CompletableFuture<Object> invoke(Map<String, ?> arguments) {
		getManager().console("Module.enusureInitialized(" + 
				"'" + arguments.get("Name") + "');");
