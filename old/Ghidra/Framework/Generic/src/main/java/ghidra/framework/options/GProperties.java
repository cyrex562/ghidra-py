 ...
 * <p> <i>Note: Names for property names are assumed to be unique. When a putXXX()
...
public class GProperties {
	...
	protected void processElement(Element elem) {
		...
		if (tag.equals("XML")) {
			...
			try {
				...
				Msg.warn(this, "Error processing array value in GProperties", exc);
			}
		}
	...
	private Object getObjectFromJson(String type, JsonElement value, JsonElement enumClass) {
		...
		switch (type) {
			...
			case G_PROPERTIES_TYPE:
			case SAVE_STATE_LEGACY_TYPE:
	...
	public JsonObject saveToJson() {
		...
		jsonObject.addProperty(PROPERTIES_NAME, propertiesName);
	...
	public GProperties getGProperties(String name) {
		return getAsType(name, null, GProperties.class);
	...
	private boolean isExpectedType(String name, Object value, Class<?> expectedType) {
		if (value != null && !expectedType.isInstance(value)) {
			Msg.debug(this,
				"Type mismatch on GProperties property \"" + name +
